...
class DatasetExport extends Command
{
    ...
    public function handle(): int
    {
        ...
        try {
            fputcsv($handle, ['recording_id', 'participant_id', 'gender', 'age_range', 'relative_audio_path', 'duration', 'sha256', 'recorded_at'], ',', '"', '');
            VoiceRecording::whereNull('deletion_requested_at')->with('participant')->chunkById(200, function ($records) use ($handle) {
                foreach ($records as $r) {
                    fputcsv($handle, [$r->id, $r->participant_id, $r->participant->gender?->value, $r->participant->age_range, $r->relative_storage_path, $r->duration_seconds, $r->sha256_checksum, $r->telegram_received_at->toIso8601String()], ',', '"', '');
                }
            });
        } finally {
            fclose($handle);
